...
var personObj = { name: "Fatima", age: 17 };
...
startCountingPromise
  ...
  function containsKey(myMap, word) {
   return myMap.has(word);
  }
  const myMap = new Map([
   ['John', 25],
   ['Alice', 30],
  ...
  const resulttt = containsKey(myMap, word);
  console.log(resulttt);
  
  function filterMap(originalMap, callback) {
   return new Map([...originalMap].filter(([word, value]) => callback(word, value)));
  }
